...
{
    internal class IdentityClearIdentityTransaction : ISpecificTransaction
    {
        ...
        public IdentityClearIdentityTransaction(IDatabaseAdapdable databaseAdapdable, Metadata metadata)
        {
        ...
        {
            var fTransfer = new TableName { ModuleName = "Identity", MethodName = "clear_identity" };
            ...
            _dbAdapter.InsertIntoCall(fTransfer, new List<TableRow> { blockHash, tid, transactionSenderKey, status, nonce, signatureKey, blocknumber });
        }
        ...
        public bool Parse(BlockHash bh, SignedBlock sb, string extrinsic)
        {
            ...
            try
            {
                ...
                if (moduleName.Equals("Identity", StringComparison.InvariantCultureIgnoreCase) &&
                    methodName.Equals("clear_identity", StringComparison.InvariantCultureIgnoreCase))
                    result = true;
            }
            catch (Exception)
            {
