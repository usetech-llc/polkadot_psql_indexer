...
{
    internal class IdentityKillIdentityTransaction : ISpecificTransaction
    {
        ...
        public IdentityKillIdentityTransaction(IDatabaseAdapdable databaseAdapdable, Metadata metadata)
        {
        ...
        {
            var fTransfer = new TableName { ModuleName = "Identity", MethodName = "kill_identity" };
            ...
            _dbAdapter.InsertIntoCall(fTransfer, new List<TableRow> { blockHash, tid, transactionSenderKey, status, nonce, signatureKey, accountRow, blocknumber });
        }
        ...
        public bool Parse(BlockHash bh, SignedBlock sb, string extrinsic)
        {
            ...
            try
            {
                ...
                if (moduleName.Equals("Identity", StringComparison.InvariantCultureIgnoreCase) &&
                    methodName.Equals("kill_identity", StringComparison.InvariantCultureIgnoreCase))
                    result = true;
            }
            catch (Exception)
            {
