...
namespace PolkaIndexer
{
    internal class StakingSetControllerTransaction : ISpecificTransaction
    {
        ...
        public void Execute(int transactionId)
        {
            ...
            var controllerRow = new TableRow
            {
                RowIndex = 1,
                RowName = "controller",
            ...
            _dbAdapter.InsertIntoCall(transfer, new List<TableRow> { blockHash, tid, signatureKey, status, nonce, transactionSenderKey, controllerRow, blocknumber });
        }
        ...
        public bool Parse(BlockHash bh, SignedBlock sb, string extrinsic)
        {
            ...
            try
            {
                ...
                if (moduleName.Equals("Staking", StringComparison.InvariantCultureIgnoreCase) &&
                    methodName.Equals("set_controller", StringComparison.InvariantCultureIgnoreCase))
                    result = true;
            }
            catch (Exception)
            {
