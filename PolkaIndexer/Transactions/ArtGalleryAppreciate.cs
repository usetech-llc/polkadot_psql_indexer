...
{
    internal class ArtGalleryAppreciate : SpecificTransaction
    {
        ...
        public ArtGalleryAppreciate(string module, string method) : base(module, method)
        {
        }
        ...
        {         
            ...
            var appreciateCall = new TableName
            {
                MethodName = "appreciate",
            ...
            var amt = Convert.ToUInt64(ArgumentExtractor.Revert(amount), 16);
            var apprRow = new TableRow
            {
                RowIndex = 1,
                RowName = "amount",
                Value = new List<string> { $"'{{{amt.ToString()}}}'" }
            };
            ...
            _dbAdapter.InsertIntoCall(appreciateCall, new List<TableRow> { blockHash, tid, status, blocknumber, nonce, signatureKey, transactionSenderKey, collectionIdRow, colRow, tokRow, apprRow });
        }
        ...
        public override bool Parse(BlockHash bh, SignedBlock sb, string extrinsic)
        {
            ...
            try
            {
                ...
                if (moduleName.Equals("ArtGalleryPallet", StringComparison.InvariantCultureIgnoreCase) && 
                    methodName.Equals("appreciate", StringComparison.InvariantCultureIgnoreCase))
                    result = true;
            }
            catch (Exception)
            {
