...
{
    internal class ArtGalleryTransfer : SpecificTransaction
    {
        ...
        public ArtGalleryTransfer(string module, string method) : base(module, method)
        {
        }
        ...
        {         
            ...
            var owner = new TableRow
            {
                RowIndex = 1,
                RowName = "tokeninfoof_owner",
                Value = new List<string> { $"'{{{recipient.ToString()}}}'" }
            };
            _dbAdapter.UpdateStorage(tokensStorage, collectionId, tokenId,  new List<TableRow> { owner });

            var transferCall = new TableName
            {
                MethodName = "transfer",
            ...
            var token = Convert.ToUInt64(ArgumentExtractor.Revert(tokenId), 16);
            var tokenIdRow = new TableRow
            {
                RowIndex = 1,
                RowName = "token_id",
                Value = new List<string> { $"'{{{token}}}'" }
            };

            var recipientRow = new TableRow
            {
                RowIndex = 1,
                RowName = "recipient",
                Value = new List<string> { $"'{{{recipient}}}'" }
            };

            _dbAdapter.InsertIntoCall(transferCall, new List<TableRow> { blockHash, tid, status, blocknumber, nonce, signatureKey, transactionSenderKey, collectionIdRow, tokenIdRow, recipientRow });
        }
