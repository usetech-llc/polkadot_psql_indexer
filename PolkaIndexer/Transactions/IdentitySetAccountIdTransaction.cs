...
{
    internal class IdentitySetAccountIdTransaction : ISpecificTransaction
    {
        ...
        public IdentitySetAccountIdTransaction(IDatabaseAdapdable databaseAdapdable, Metadata metadata)
        {
        ...
        public void Execute()
        {
            var fTransfer = new TableName { ModuleName = "Identity", MethodName = "set_account_id" };
            ...
            _dbAdapter.InsertIntoCall(fTransfer, new List<TableRow> { transactionSenderKey, status, nonce, signatureKey, newValueRow, indexRow, blocknumber });
        }
        ...
        public bool Parse(SignedBlock sb, string extrinsic)
        {
            ...
            try
            {
                ...
                if (moduleName.Equals("Identity", StringComparison.InvariantCultureIgnoreCase) &&
                    methodName.Equals("set_account_id", StringComparison.InvariantCultureIgnoreCase))
                    result = true;
            }
            catch (Exception)
            {
