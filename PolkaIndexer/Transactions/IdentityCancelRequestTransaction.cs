...
{
    internal class IdentityCancelRequestTransaction : ISpecificTransaction
    {
        ...
        private string sk;
        private BigInteger regIndex;

        public IdentityCancelRequestTransaction(IDatabaseAdapdable databaseAdapdable, Metadata metadata)
        {
        ...
        public void Execute(int transactionId)
        {
            ...
            var regInd = new TableRow
            {
                RowIndex = 0,
                RowName = "reg_index",
                Value = new List<string> { regIndex.ToString() }
            };
            ...
            _dbAdapter.InsertIntoCall(fTransfer, new List<TableRow> { blockHash, tid, transactionSenderKey, status, nonce, signatureKey, regInd, blocknumber });
        }
        ...
        public bool Parse(BlockHash bh, SignedBlock sb, string extrinsic)
        {
            ...
            regIndex = Scale.DecodeCompactInteger(ref parse).Value;
            ...
            try
            {
                ...
                if (moduleName.Equals("Identity", StringComparison.InvariantCultureIgnoreCase) &&
                    methodName.Equals("cancel_request", StringComparison.InvariantCultureIgnoreCase))
                    result = true;
            }
            catch (Exception)
            {
