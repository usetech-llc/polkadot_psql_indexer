...
{
    internal class ArtGalleryBurn : SpecificTransaction
    {
        ...
        public ArtGalleryBurn(string module, string method) : base(module, method)
        {
        }
        ...
        {         
            ...
            _dbAdapter.DeleteFromStorageByDoubleKey(tokensStorage, $"'{{{colId.ToString()}}}'", $"'{{{tokId.ToString()}}}'");


            var burnCall = new TableName
            {
                MethodName = "burn",
            ...
            _dbAdapter.InsertIntoCall(burnCall, new List<TableRow> { blockHash, tid, status, blocknumber, nonce, signatureKey, transactionSenderKey, collectionIdRow, tokenIdRow });
        }
        ...
        public override bool Parse(BlockHash bh, SignedBlock sb, string extrinsic)
        {
            ...
            try
            {
                ...
                if (moduleName.Equals("ArtGalleryPallet", StringComparison.InvariantCultureIgnoreCase) && 
                    methodName.Equals("burn", StringComparison.InvariantCultureIgnoreCase))
                    result = true;
            }
            catch (Exception)
            {
